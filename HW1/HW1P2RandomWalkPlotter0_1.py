...
def plot_xyz_positions(x_arr, y_arr, z_arr):
	"""
	color_arr = np.linspace(0, len(x_arr))
	min_zarr = min(z_arr)
	max_zarr = max(z_arr)
	print("min: %f " % min_zarr)
	print("max: %f " % max_zarr)
	colorzarr = []
	for i in range(0, len(z_arr)):
		zvalue = ((z_arr[i]+min_zarr)/(max_zarr + min_zarr))
		colorzarr.append(zvalue)

	print("min: %f " % min(colorzarr))
	print("max: %f " % max(colorzarr))
	"""
	...
	#plt.colorbar(z_arr)
	fignamestring = "X_YpositionsJan24.png"
...
def plot_xyz_displacement(displacement_arr, radius_arr):
	...
	fignamestring = "DisplacementVstepJan24.png"
	plt.savefig(fignamestring)
        print("Saving Plot: %s " % fignamestring)
	return 0
